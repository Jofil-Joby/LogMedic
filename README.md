...
> A portable engineering agent for **application logging hygiene**.

LogMedic inspects observable project evidence, detects **missing recognizable logging artifacts**, and produces an explainable improvement plan. Its purpose is not to replace specialist tooling. It provides a focused, auditable diagnostic layer that can travel across agent runtimes.
...
| Layer | LogMedic behavior |
| --- | --- |
| Domain | application logging hygiene |
| Primary signal | log-related project files |
| Remediation | Provide structured application logging |
...
LogMedic is a focused diagnostic prototype. Its conclusions are limited to the evidence and rules implemented in this repository. It should complement, not replace, production-grade static analysis, security scanners, observability platforms, CI systems, or human review where appropriate.
